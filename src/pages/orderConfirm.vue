          ...
          <div class="item-good">
            ...
            <ul>
              <li v-for="(item,index) in cartList" :key="index">
                <div class="good-name">
                  <!-- 图片是后台用了cdn加速器,加速页面加载，拉取一次永久缓存，如不用cdn会用纯服务器去拉取图片，比较耗时，增加请求 -->
...
export default{
  ...
  methods:{
    ...
    editAddressModal(item){
      this.userAction = 1;
      this.checkedItem = JSON.parse(JSON.stringify(item));//修改地址信息的时候，把item赋值给了checkedItem，因为item是引用类型，在添加modal里修改表单项的值时，由于v-model，地址栏里的数据也会实时变化，这里可以用深拷贝复制一下item再赋给checkedItem
      this.showEditModal = true;
    },
    delAddress(item){
      this.userAction = 2;
    ...
    submitAddress(){
      ...
      if(userAction == 0 || userAction ==1){
        ...
        if(!receiverName){
          errMsg = '请输入收货人名称';
        }else if(!receiverMobile || !/^((13[0-9])|(14[5|7])|(15([0-3]|[5-9]))|(18[0,5-9]))\d{8}$/.test(receiverMobile)){
          errMsg = '请输入正确格式的手机号';
          ...
          errMsg = '请输入收货地址';
        }else if(!/^[1-9]\d{5}(?!\d)$/.test(receiverZip)){
          errMsg = '请输入六位邮编';
        }
        if(errMsg){
    ...
    getCartList(){
      this.axios.get('/carts').then((res)=>{
        ...
        this.cartList = list.filter(item=>item.productSelected);//过滤选中的列表，获得一个全新的list
...
<style lang="scss">
  .order-confirm{
    ...
    .edit-wrap{
        ...
        .item{
          margin-bottom: 15px;
          .input{//在定义base.scss已指定border-sizing:border-box,
            display: inline-block;
            width: 283px;
            height: 40px;
            line-height: 40px;
